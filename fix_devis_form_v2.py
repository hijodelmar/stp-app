...
file_path = 'd:/websites/stp/templates/devis/form.html'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Define the broken pattern (allowing for some whitespace variations)
# We look for "let lineIndex = {{ form.lignes| length" followed by newlines and closing braces
broken_pattern = r'let lineIndex = \{\{ form\.lignes\|\s*length\s*\n\s*\}\}\s*\n\s*\}\;'

# Actually, the view_file showed:
# let lineIndex = {{ form.lignes| length
#             }
#         };
# It's better to match the exact string shown in view_file if possible, or use a broad regex.

# Let's try matching the specific context that keeps breaking.
target_str = """            if (addButton) {
                let lineIndex = {{ form.lignes| length
            }
        };
...
if target_str in content:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Fixed using exact string match")
else:
    # If exact match fails (whitespace?), try a more aggressive normalization or regex
    print("Exact match failed, trying normalized replacement")
    
    # Normalize whitespaces to single spaces for searching
    # This is risky if we replace wrong things, but the code block is specific.
    # Alternatively, just look for the specific broken line.
    
    broken_line = "let lineIndex = {{ form.lignes| length"
    if broken_line in content:
        # Find where it is
        idx = content.find(broken_line)
        # Check if matched
        if idx != -1:
             # We assume the next few lines are garbage braces.
             # We will just rewrite the whole file content based on known good structure if this fails.
             print("Found broken line, but simple replace safer.")
             pass

    # Fallback: Rewrite the whole file again with the KNOWN GOOD CONTENT.
    # This has worked before.
    print("Falling back to full rewrite to be safe.")
    
    # ... I will just include the full rewrite logic here as it is the most robust.
    # (Content from previous step that was good)
    
    full_content = r"""{% extends 'base.html' %}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateLineVisibility(row) {
            ...
            if (category === 'prestation') {
                ...
                qteInput.value = 1; 
                if (desInput) desInput.value = ''; // Clear designation as it is not used
